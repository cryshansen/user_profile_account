...
(function (Drupal, drupalSettings) {
  Drupal.behaviors.salesComparisonChart = {
    attach: function (context, settings) {
      ...
      const currentMontxh = drupalSettings.user_profile_sales.current_month;
      const previousMonth = drupalSettings.user_profile_sales.previous_month;

      // Extract labels (assume both weeks use same days for simplicity)
      const labels6 = currentMonth.map(item => item.day);
      const currentSales6 = currentMonth.map(item => item.total_sales);
      const previousSales6 = previousMonth.map(item => item.total_sales);
